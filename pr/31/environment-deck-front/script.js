...
async function downloadMultipleEnvironmentDeckImages() {
  let jsonEntries;
  try {
  ...
  try {
    ...
    const zipName = sanitizeBatchFilename(jsonEntries[0]?.['Originating deck'] || 'environment-deck-front-images');
...
$('#downloadMultipleButton').on('click', function () {
  downloadMultipleEnvironmentDeckImages();
});

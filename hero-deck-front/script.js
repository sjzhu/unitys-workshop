...
function drawCardCanvas() {
  ...
  if (loadedUserImages[ADDITIONAL_ICON]) {
    drawArtInCroppedArea('hdcf_additionalIcon');
  }
...
async function downloadMultipleHeroDeckImages() {
  let jsonEntries;
  try {
  ...
  try {
    ...
    const zipName = sanitizeBatchFilename(jsonEntries[0]?.['Originating deck'] || 'hero-deck-front-images');
...
$('#downloadMultipleButton').on('click', function () {
  downloadMultipleHeroDeckImages();
});
